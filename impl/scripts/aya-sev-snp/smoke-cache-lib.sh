...
twod_hsm_nix_outlink_hit() {
  ...
  case "$attr" in
    ...
    disk-production | disk-production-lab | disk-production-lab-selftest | \
      disk-production-lab-quote-smoke)
      ...
      # disk-production-lab-quote-smoke adds the TASK-7.7 (4c) quote-smoke oneshot — the build
      # stamp already hashes all enclave-protocol src/*.rs + Cargo.* + the smoke scripts, so
      # editing the smoke Rust auto-invalidates the cached image).
      local c
