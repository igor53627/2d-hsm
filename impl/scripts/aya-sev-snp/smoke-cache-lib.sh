...
twod_hsm_nix_outlink_hit() {
  ...
  case "$attr" in
    ...
    disk-production | disk-production-lab | disk-production-lab-selftest | \
      ...
      disk-production-lab-agent-keygen-smoke | disk-production-lab-agent-faucet-smoke)
      ...
      # disk-production-lab-agent-faucet-smoke adds the TASK-15 faucet WRITE-path serve unit
