...
let
  ...
  buildFeatures =
    if staging then [ "staging-vsock" ]
      ...
      [ "agent-gateway" "vsock-transport" "platform-root-from-boot-file" "agent-keygen-exec-preview" "agent-configure-treasury-preview" "agent-sign-faucet-preview" "agent-sign-transfer-preview" "agent-backup-export-preview" "agent-prove-identity-preview" ]
