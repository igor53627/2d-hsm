...
//!
//! Production TEE replaces [`seal_mldsa65_secret_key_v0`] / [`unseal_mldsa65_secret_key_v0`]
//! with platform sealing (vTPM, SNP VMPL, etc.). The v0 XOR stream here is **reference-only**
//! and must not be used for real key material.
...
pub fn is_sealed_signer_installed() -> bool {
    INSTALLED_SIGNER
        .lock()
        .expect("pq signer mutex poisoned")
        .is_some()
}
...
pub fn reset_installed_pq_signer_for_tests() {
    *INSTALLED_SIGNER.lock().expect("pq signer mutex poisoned") = None;
}
...
pub fn seal_mldsa65_keypair_v0(
    ...
    let mut out = Vec::with_capacity(11 + enclave_measurement.len() + cipher.len());
...
pub fn seal_mldsa65_secret_key_v0(
    ...
    .map_err(|_| ProtocolError::PqSigningUnavailable("reference public key testvector missing"))?;
    seal_mldsa65_keypair_v0(secret_key, &pk, enclave_measurement)
}
...
pub fn unseal_mldsa65_keypair_v0(
    ...
    if sealed_blob.len() < 11 + payload_len {
