...
| `agent_backup_v1.{bin,json}` | **TASK-13b slice 3** frozen `pq-agent-backup-v1` DR-backup blob — the byte-exact ML-KEM-1024 KEM-DEM ENVELOPE. Frozen v1 byte order (big-endian; `lp16`/`lp32` = u16/u32 length-prefix): `magic(8) ‖ backup_format_version(u16) ‖ lp16(recovery_key_id) ‖ chain_id(u64) ‖ lp16(environment_identifier) ‖ kem_ct(1568) ‖ lp32(key_refs_manifest) ‖ payload_nonce(12) ‖ lp32(dem_ciphertext)`. **`AAD` = the exact serialized header bytes (`magic` through `payload_nonce` inclusive, INCLUDING the length prefixes) — CWE-347-unambiguous;** see `backlog/docs/agent-gateway-keystore-backup-format.md`. Wrapped to the `agent_backup_recovery_keypair_v1` encaps key; `chain_id=11565`, `environment_identifier=env-prod-0`. The **payload is an opaque slice-1 stand-in** (its restorable contents are defined in slice 4) — this vector freezes the ENVELOPE wire format, not the payload semantics. Consumed by the `agent_backup` byte-exact freeze + offline-open round-trip. **TEST KEYS ONLY.** Regen: `cargo test --features agent-backup-export-preview regen_agent_backup_golden_vector -- --ignored --nocapture`. |
| `agent_backup_recovery_keypair_v1.{encaps.bin,decaps.bin}` | The ML-KEM-1024 operator recovery keypair the `agent_backup_v1` blob is wrapped to: `encaps.bin` = the 1568-byte encapsulation (public) key the enclave seals; `decaps.bin` = the 64-byte keypair SEED (`from_seed` reconstructs the decapsulation key — the OFFLINE recovery secret). A consumer reconstructs the decaps key from the seed and opens the blob to prove the KEM-DEM round-trip + AC#13 DR-independence. **TEST KEYS ONLY — the decaps seed is the public constant `[0x5a;64]`; in production the matching ML-KEM private key NEVER enters a runtime TEE.** |
| `restore_ingress_envelope_v1.{bin,json}` | **TASK-24 AC#1** frozen `2d-hsm-agent-restore-ingress-v1` ENVELOPE — the attested SECOND KEM-DEM layer of the recovery ceremony (the operator's offline re-wrap of the `restore_ingress_v1.bin` PAYLOAD to the destination TEE's EPHEMERAL ML-KEM-1024 key). Frozen v1 byte order (big-endian): `magic(8)=2DAGRIE\0 ‖ restore_ingress_envelope_format_version(u16) ‖ lp16(dest_measurement) ‖ chain_id(u64) ‖ lp16(environment_identifier) ‖ manifest_hash(32) ‖ original_backup_digest(32) ‖ ingress_kem_ct(1568) ‖ ingress_nonce(12) ‖ lp32(dem_ciphertext)`. **`AAD'` = the exact serialized header bytes (`magic` through `ingress_nonce` inclusive, INCLUDING length prefixes + nonce) — the SAME CWE-347 discipline as `agent_backup_v1`;** `manifest_hash = SHA3-256("…-manifest-hash" ‖ 0x00 ‖ manifest)`, `original_backup_digest = SHA3-256("…-backup-digest" ‖ 0x00 ‖ agent_backup_v1.bin)`, `ingress_key = SHA3-256("2d-hsm-agent-restore-ingress-v1" ‖ 0x00 ‖ ss')` — all three ingress domains structurally prefix-free via the `0x00` separator after the ASCII domain tag. `ingress_nonce` fixed-zero (safe: `ss'` fresh per `Encaps`). Cross-references BOTH prior goldens (wraps `restore_ingress_v1.bin`; digests `agent_backup_v1.bin`). **TEST KEYS ONLY.** Regen (mints all 4 files): `cargo test --features agent-backup-export-preview regen_restore_ingress_envelope_golden_vector -- --ignored --nocapture`. |
| `restore_ingress_dest_ephemeral_keypair_v1.{encaps.bin,decaps.bin}` | The ML-KEM-1024 DESTINATION EPHEMERAL keypair the `restore_ingress_envelope_v1` blob is re-wrapped to: `encaps.bin` = the 1568-byte attested ephemeral public key the operator re-wraps to; `decaps.bin` = the 64-byte keypair SEED. A consumer reconstructs the decaps key from the seed and opens the envelope to prove the ceremony KEM-DEM round-trip. **TEST KEYS ONLY — the decaps seed is the public constant `[0x6c;64]`; in the LIVE ceremony this key is GENERATED inside the destination TEE and never exists off-device (the fixture exists only so the golden can be opened offline).** |
