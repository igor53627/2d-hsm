...
| `agent_backup_v1.{bin,json}` | **TASK-13b slice 3** frozen `pq-agent-backup-v1` DR-backup blob — the byte-exact ML-KEM-1024 KEM-DEM ENVELOPE (`magic 2DAGTBK\0` ‖ `backup_format_version=1` ‖ length-prefixed header ‖ `kem_ct(1568)` ‖ `payload_nonce(12)` ‖ `dem_ciphertext`). **`AAD` = the exact serialized header bytes incl. length prefixes (CWE-347-unambiguous);** see `backlog/docs/agent-gateway-keystore-backup-format.md`. Wrapped to the `agent_backup_recovery_keypair_v1` encaps key; `chain_id=11565`, `environment_identifier=env-prod-0`. The **payload is an opaque slice-1 stand-in** (its restorable contents are defined in slice 4) — this vector freezes the ENVELOPE wire format, not the payload semantics. Consumed by the `agent_backup` byte-exact freeze + offline-open round-trip. **TEST KEYS ONLY.** Regen: `cargo test --features agent-backup-export-preview regen_agent_backup_golden_vector -- --ignored --nocapture`. |
