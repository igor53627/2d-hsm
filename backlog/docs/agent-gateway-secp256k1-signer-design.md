...
> **Concrete format spec:** the sealed keystore (`pq-agent-keystore-v1`) and DR backup blob (`pq-agent-backup-v1`) byte layouts, KDF domains, ML-KEM backup wrapping, restore scope, and test-vector requirements are specified in `agent-gateway-keystore-backup-format.md` (TASK-7.2). This section is the high-level model.
...
> **Anti-rollback mechanism spec:** the selected mechanism (remote monotonic counter + epoch-lease), its seal-before-emit / boot-restore binding, the active-active rule, and the AC#5 production-funding block are specified in `agent-gateway-anti-rollback.md` (TASK-7.7). This paragraph is the high-level residual statement.
