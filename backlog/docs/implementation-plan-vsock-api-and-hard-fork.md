...
2. **TASK-1 staging transport + platform boot hook (PR #4, in progress)** — items 2–3 shipped together: `staging-host` / `enclave-uds-staging`, `TWOD_HSM_ENCLAVE_STAGING_SOCKET` (legacy `2D_HSM_*`), `boot_configure_pq_seal_v1_platform_root`, optional `platform-provisioning-from-file` (labs), **`release_build` compile_error** on reference/staging features.
