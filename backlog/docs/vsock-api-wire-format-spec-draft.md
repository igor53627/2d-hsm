...
see `authorization-tickets-precompile-spec-draft.md`). Because the domains differ, a cross-domain
preimage collision is computationally infeasible under Keccak's preimage/collision resistance, so
neither signature validates under the other's verifier (a `ticket_hash` cannot be passed off as a
block-root preimage, or vice-versa). The 2D verifier MUST reproduce this exact block-root preimage.
...
- The host supplies only the 32-byte `block_hash`; the enclave fixes the domain prefix and signs the
  derived hash.
- **Precedence:** the stateful dispatcher rejects an unarmed caller with an error directing it to
  `ARM_FOR_PRODUCTION` first, before the handler's `pq_signing_ready` check runs — an unarmed caller
  never learns signer presence.
...
- **`scope_class` policy per op (TASK-18 18-5 completeness audit).** The 18-2 `scope_identity`
  ...
  | `PUBLIC_IDENTITY`, `PROVE_IDENTITY`, `GET_RESTORE_PUBKEY` | n/a (no capability) | n/a | low-privilege reads / ceremony-setup (GET_RESTORE_PUBKEY publishes the attested ephemeral pubkey — no sealed-state mutation, no spend authority). |
