...
sensitive ops — PARTIAL today (2 live ops enforce `scope_class != 0`: treasury-keygen + treasury-config;
full closure PENDING the 18-5 completeness audit across every budget-sensitive opcode, incl. the still-preview-banned SIGN_FAUCET_DISPENSE / EXPORT_BACKUP)). **NB: the guard is conditional on provenance** — it is security theater unless
...
**`SIGN_FAUCET_DISPENSE` carve-out (no-capability committed op, slice 15-3b).** SIGN_FAUCET_DISPENSE is the one *committed* (EpochOnly) op that carries **no capability** (it is a runtime-signing opcode, §10.3), so for it there is no capability **counter** sequencer and no capability **signature** binding the `request_id` — the `request_id` is the raw host-supplied envelope key 4, and **per-op uniqueness is the host/orchestrator's responsibility** (the enclave never enforces it). This is **safe** because it is fail-closed in the same direction as the admin case: a reused (or empty) request_id whose proposed `{epoch, structural, marks}` differs from the recorded one is conflict-rejected → the enclave fails closed (0x46, no seal/emit) — never a double-advance or double-spend. The no-double-spend property rests entirely on the **request_id-alone keying + the anchor conflict-reject + the per-op fresh nonce + the marks comparison** (rules 1–3 above), NOT on the request_id being signature-bound; binding is what an admin op additionally gets for *replay* protection of a captured capability, which a no-capability faucet op has no capability to replay. The residual is therefore a host-inducible *availability* gap (a host that always reuses one request_id completes at most one dispense then self-wedges into 0x46), not a custody risk — consistent with the host-liveness-DoS residuals already accepted in §7.
...
`agentAntiRollbackMode ? "none"` param + the derived `agentAntiRollbackEnabled` (`= agentTransferFaucetSignerPackage != null`, so a funding profile can't bypass) + `antiRollbackResidualOptOut ? false`; `nixos-module.nix` adds the assertion below as an ALWAYS-PRESENT assertion (NOT inside the `lib.optionals isProd` list — the gate predicate, derived from the single-source `ac5-funding-gate.nix`, is self-guarded by its own `productionMode &&` term) PLUS an always-present `productionMode ⇒ isProd` coupling invariant that closes the isProd/productionMode decoupling (so a productionMode build on a non-prod guestProfile can't silently drop the isProd-gated mainnet + funding assertions); `flake.nix` adds `checks.agent-anti-rollback-gate` exercising both polarities + the derivation, wired into CI alongside the mainnet gate. No funding profile ships yet (TASK-15), so the assertion is a dormant tripwire on every output and the flake check is where it is verified. The `agentAntiRollbackMode` is a BUILD-TIME guest-profile param captured in the measured image — NOT a runtime env channel a host can flip — so the §1406 env-channel measured-boot obligation is satisfied by construction for the mode itself; the broader disk-image measurement coverage is the TASK-1.1 chain. The lab-stub-endpoint-counts-as-none `usesLab` downgrade is forward-declared for when a real anti-rollback endpoint + its lab fixture land with TASK-15.)* (modeled on the existing `productionMode` build-gate pattern — e.g. the mainnet
`!(productionMode && labFixtures)` gate; but per the IMPLEMENTED note above the AC#5 assertion is ALWAYS-PRESENT, not inside `lib.optionals isProd`). Add a guest-profile param
