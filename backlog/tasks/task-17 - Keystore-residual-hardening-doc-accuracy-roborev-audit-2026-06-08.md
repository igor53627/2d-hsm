...
- [ ] #2 Doc: max_batch_size enforcement wording corrected. agent-gateway-keystore-backup-format.md:90 says 'max_batch_size + total_capacity enforced before seal' but MAX_BATCH_SIZE (agent_keystore.rs:236) is declared-and-unused; the actually-binding limits at seal are MAX_TOTAL_KEY_ENTRIES (count, :457) and MAX_KEYSTORE_BLOB_SIZE/framing-reserve (size, :533). Either reconcile the doc to current enforcement (batch cap applies at GENERATE_KEYS batch time, deferred) or implement the batch-size check, and name the blob-size ceiling as the binding limit.
