...
- [ ] **Cleanup policy (gemini HIGH: PID-unique names make the never-cleaned dir grow unboundedly across runs):** both exchange files are deleted after each successful exchange, AND the test harness removes any leftover `input-*`/`output-*` files at process start (crash leftovers from other pids are bounded by the sweep, not accumulated).
