...
5. **Boot wiring** — SNP-quote fetch + host relay + verify/reconcile into boot/install via `verify_anchor_response_bytes`; per-op epoch advance + anchor ack ATOMIC with the structural bump (makes structural_version load-bearing); partial-init must not expose rollback-sensitive commands.
