...
2. **Nonce plumbing** — **LANDED (`src/agent_challenge.rs`)**: enclave freshness-challenge state machine — `issue_challenge` (CSPRNG draw, overwrite-on-reissue) into a volatile process-global; `Challenge::report_data()` binds the same draw into the SNP report_data; `consume_outstanding_challenge() -> Option<Challenge>` single-use take; volatile-only enforced structurally (non-Serialize); poison-recover. 9 tests incl. single-use-replay-is-none + entropy. DEFERRED to boot-wiring: actual SNP-quote bytes fetch + host relay + the atomic peek→verify→consume sequencing (issue runs after unseal; every verify outcome routes through consume + assert returned-nonce==verified).
