...
| **3** | SNP launcher + real TEE measurement | **In progress** — AC#4 + AC#5 **done & live on aya** (`.#disk-production-lab` boots under SEV-SNP; real 48B measurement, bytes=3210, pq_ready=true); manifest split **done** (schema v2: build-identity vs `tee_measurement`); only VCEK→ASK→ARK cert-chain (auxblob) + verifier policy spec remain |
